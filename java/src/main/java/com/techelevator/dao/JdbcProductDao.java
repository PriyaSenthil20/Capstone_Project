...
public class JdbcProductDao implements ProductDao {
    ...
    @Override
    public List<Product> getAvailableProductsByTypeAndSize(int typeId, int sizeId) {
        List<Product> products = new ArrayList<>();
        String sql  = selectProductSql + " FROM Products " +
                "WHERE product_available = true AND product_type_id = ? AND size_id = ?";
        try {
            SqlRowSet results = jdbcTemplate.queryForRowSet(sql, typeId, sizeId);
            while (results.next()) {
                Product product = mapRowToProduct(results);
                products.add(product);
            }
